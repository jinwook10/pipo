		...
		<div class="bs-example" data-example-id="striped-table">
			<table class="table table-striped">
				...
				<tbody>
					<c:forEach var="bbsvo" items="${bbsList}" varStatus="status">
						<tr>
							<td>${status.count+pagevo.startNo}</td>
							<td><a
								href="<%=request.getContextPath() %>/view?no=${bbsvo.no}">${bbsvo.title}</a></td>
							...
							<td>
								...
								<a href="<%=request.getContextPath() %>/delno?no=${bbsvo.no}">삭제</a>
							</td>
							</c:if>
						</tr>
					</c:forEach>
			...
			<div id="bs-example">
				<c:if test="${pagevo.prev}">
					<a
						href="<%=request.getContextPath() %>/bbslist?page=${pagevo.startPage-1}">[prev]</a>
				</c:if>
				<c:forEach begin="${pagevo.startPage }" end="${pagevo.endPage }"
					var="idx">
					<a href="<%=request.getContextPath() %>/bbslist?page=${idx }">[${idx}]</a>
				</c:forEach>
				<c:if test="${pagevo.next}">
					<a
						href="<%=request.getContextPath() %>/bbslist?page=${pagevo.endPage+1}">[next]</a>
				</c:if>
			</div>
			...
			<c:if test="${nowuser != null }">
				<div id="btndiv">
					<a class="btn btn-primary"
						href="<%=request.getContextPath()%>/bbswrite">글쓰기</a>
					<!-- 	<a class="btn btn-primary"
					>Test</a> -->
				</div>
			</c:if>
